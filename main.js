...
function showSide() {
  $('#content').animate({ width: '90%' }, { queue: false, duration:300 });
  $('#leftside').animate({ width: '10%' }, { queue: false, duration:300 });
}
...
function varDump(/* ... */) {
  /**
 * Recursao do metodo varDump
 * @param midex item Qualquer valor
 * @param int nivel Nivel de indentacao
 * @return string Informacoes do valor
 */
  this.var_dump_rec = function(item, nivel) {
    if (varDump.max_iteracoes > 0 && varDump.max_iteracoes < nivel) {
      return this.indentar(nivel) + "*MAX_ITERACOES(" + varDump.max_iteracoes+ ")*\n";
    }
    if (item === null) {
    ...
    switch (tipo) {
      case 'object':
        ...
        switch (classe) {
          ...
          break;
          default:
            str += /*this.indentar(nivel) + "obj(" + classe + ") */"{\n";
            var exibiu = false;
            for (var i in item) {
              exibiu = true;
              str += this.indentar(nivel + 1) + "[" + i + "] => ";
        ...
        break;
        case 'number':
          str += /* this.indentar(nivel) + */ "int(" + item.toString() + ")\n";
        break;
        case 'string':
          str += /* this.indentar(nivel) + */ "str(" + item.length + ") \"" + item + "\"\n";
        break;
        case 'boolean':
          str += /* this.indentar(nivel) + */ "bool(" + (item ? "true" : "false") + ")\n";
        break;
        case 'function':
          ...
          str += this.indentar(nivel + 1) + "obj(prototype) {\n";
...
varDump.prototype.max_iteracoes = 0;
